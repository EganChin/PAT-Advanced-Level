...
//2019.4.8 16:43 reading
//2019.5.15 22:46 read
...
//各个学生选择的课程，学生id最大数为26*26*26*10
vector<int> selectCourses[26*26*26*10+1];
...
int main(){
    ...
    for(int i=0; i<k; i++){
        ...
        for(int j=0; j<stunum; j++){
            char name[4];
            scanf("%s", name);
            selectCourses[getId(name)].push_back(cid);
        }
    }
    ...
    for(int i=0; i<n; i++){
        ...
        //将课程号排序
        sort(selectCourses[id].begin(), selectCourses[id].end());

        //输出姓名
        printf("%s %d", name, selectCourses[id].size());

        //输出所选课程
        for(int j=0; j<selectCourses[id].size(); j++)
            printf(" %d", selectCourses[id][j]);
